...
def get_resampler(strategy: str):
    ...
    if strategy == "smote":
        ...
        raise ValueError(f"Unknown sampling strategy: {strategy}. " "Choose from: smote, undersample, combined, none")


def apply_resampling(X, y, strategy: str):
    """Apply resampling and log class distribution before/after."""
    logger.info(f"Before resampling → " f"class 0: {(y==0).sum()} | class 1: {(y==1).sum()}")
    ...
    logger.info(f"After resampling  → " f"class 0: {(y_res==0).sum()} | class 1: {(y_res==1).sum()}")
...
def find_best_threshold(model, X_val, y_val):
    ...
    logger.info(f"Best threshold = {best_threshold:.4f} " f"(F1 class=1: {best_f1:.4f})")
...
def evaluate_model(model, X_val, y_val, threshold=None):
    ...
    logger.info("\n" + cast(str, classification_report(y_val, y_pred, zero_division=0)))
...
def train_all_models(args):
    ...
    logger.info(f"Class balance → " f"0={(y_train==0).sum()} " f"1={(y_train==1).sum()}")
    ...
    for model_name, model in models.items():
        ...
        with mlflow.start_run(run_name=model_name):
            ...
            for fold, (train_idx, cv_idx) in enumerate(cv.split(X_train, y_train), start=1):
                X_tr = X_train[train_idx]
                y_tr = y_train[train_idx]
                X_cv = X_train[cv_idx]
                y_cv = y_train[cv_idx]

                X_tr_res, y_tr_res = apply_resampling(X_tr, y_tr, SAMPLING_STRATEGY)
            ...
            logger.info(f"CV Mean ROC-AUC={avg_cv_auc:.4f} | " f"CV Mean Avg Precision={avg_cv_ap:.4f}")

            X_train_res, y_train_res = apply_resampling(X_train, y_train, SAMPLING_STRATEGY)
            model.fit(X_train_res, y_train_res)

            metrics, best_threshold = evaluate_model(model, X_val, y_val, threshold=None)
            ...
            log_model_to_mlflow(model, model_name, X_train[:5], use_registry)
    ...
    logger.info("=" * 60)
    logger.info(f"Best model: {best_model_name} " f"Avg Precision={best_avg_precision:.4f}")

    if best_model_name is None:
        logger.warning("No models were trained — skipping best-model selection.")
...
if __name__ == "__main__":
    ...
    logger.info(f"Training complete → " f"{best_name} Avg Precision={best_ap:.4f}")
